...
class BlackjackEnv(gym.Env):
    ...
    def __init__(self, natural=False, nb_deck=1):
        ...
        self.seed()
        self.deck_value = 0
        self.deck_cards_counting = np.zeros(10)
    ...
    def draw_card(self):  # randomly draw a card
        ...
        self.deck_cards_counting[card - 1] += 1
        if (card in [1,10]) :
            self.deck_value -= 1
    ...
    # This enables to count card with the Hi-Lo strategy
    def get_deck_value(self) :
        #return self.deck_value / ( (len(self.deck) // 52) + 1 )
        return self.deck_value

    # This enables to count every card
    def get_deck_cards_counting(self) :
        return self.deck_cards_counting
